...
@Repository
public class CentroCustoJdbcRepository implements ImportJdbcRepository {

    private static final Logger log = LoggerFactory.getLogger(CentroCustoJdbcRepository.class);

    private final JdbcTemplate jdbcTemplate;

    public CentroCustoJdbcRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    @Override
    public DestinoImportacaoEnum getDestino() {
        return DestinoImportacaoEnum.CENTRO_CUSTO;
    }
    ...
    public Set<String> findExistingKeys(List<CommandDTO> commands) {
        ...
        String sql = String.format(
                SqlStatements.SELECT_EXISTENTES_CENTRO_CUSTO,
                sistemaPlaceholders,
                filialPlaceholders,
                codigoPlaceholders
        );
        ...
        if (log.isDebugEnabled()) {
            log.debug("SQL findExistingKeys centro_custo ->\n{}", sql);
    ...
    public int[] batchInsert(final List<CommandDTO> commands) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("SQL batchInsert centro_custo ->\n{}", SqlStatements.INSERT_CENTRO_CUSTO);
    ...
    public int[] batchUpdate(final List<CommandDTO> commands) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("SQL batchUpdate centro_custo ->\n{}", SqlStatements.UPDATE_CENTRO_CUSTO);
    ...
    public int insert(CommandDTO command) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("SQL insert individual centro_custo ->\n{}", SqlStatements.INSERT_CENTRO_CUSTO);
    ...
    public int update(CommandDTO command) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("SQL update individual centro_custo ->\n{}", SqlStatements.UPDATE_CENTRO_CUSTO);
